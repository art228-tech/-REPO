...
async def on_scan_start(call: CallbackQuery, ctx: BotContext) -> None:
    ...
    if not (
        ...
        await call.answer(
            "В настройках выключены все источники сбора.", show_alert=True
        )
        return
    ...
    try:
        await ctx.scan.start(resume=resume, on_progress=reporter)
...
async def on_scan_stop(call: CallbackQuery, ctx: BotContext) -> None:
    ...
    await call.message.edit_text(
        text, reply_markup=main_menu(await ctx.has_account(), False)
    )
    await call.answer()
...
async def on_scan_status(call: CallbackQuery, ctx: BotContext) -> None:
    ...
    if ctx.scan.last_error:
        await call.message.edit_text(
            ctx.scan.last_error, reply_markup=back_to("menu:main")
        )
    elif ctx.scan.last_report is not None:
        await call.message.edit_text(
            format_report(ctx.scan.last_report), reply_markup=back_to("menu:main")
        )
    else:
        await call.message.edit_text(
            "Обходов пока не было.", reply_markup=back_to("menu:main")
        )
    await call.answer()
