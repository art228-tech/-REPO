...
class Config:
    ...
    # Три ролика из одного набора материалов вместо одного: обычный кадр и два
    # со сдвигом рамки вбок. Треть кадра уходит, треть приходит с той стороны,
    # которая в обычном ролике обрезана.
    three_frames: bool = False
