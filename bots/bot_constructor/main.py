...
from config import BOT_TOKEN, DB_PATH, MEDIA_DIR, WEBAPP_HOST, WEBAPP_PORT, WEBAPP_URL
from database import init_db
from bots.manager import get_manager
from handlers import register_all
from utils.helpers import rehost_step_photos
...
async def main() -> None:
    ...
    try:
        ...
        async def _rehost_loop() -> None:
            while True:
                try:
                    await rehost_step_photos(bot, MEDIA_DIR)
                except Exception as e:
                    log.warning("rehost loop: %s", e)
                await asyncio.sleep(60)
        asyncio.create_task(_rehost_loop())
