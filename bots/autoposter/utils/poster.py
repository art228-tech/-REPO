...
from utils.link_check import repair_post_links
...
class PosterEngine:
    ...
    async def _post_one(self, st) -> None:
        ...
        # проверка t.me-ссылок и автоподмена через запасной пул
        try:
            from database import get_db as _gd
            # main_bot не передан напрямую — берём из менеджера? нет, его нет здесь.
            # Достаём основной бот из manager или передадим. Используем helper-бот
            # для проверок было бы неверно (другая личность), поэтому проверяем
            # через любого из помощников — у Bot API get_chat от любого бота
            # возвращает то же самое.
            check_status = await repair_post_links(_gd(), bot, post, ADMIN_IDS)
        except Exception as _e:
            log.warning("repair_post_links: %s", _e)
            check_status = "ok"
        if check_status == "blocked":
            await get_db().set_posting_state(ch_id, is_running=0)
            return
        # перечитаем пост после возможной подмены
