"""Управление помощниками — добавление по токену, удаление."""
...
async def cb_h_add(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    await cb.message.edit_text(
        ...
        "Помощник должен быть админом нужного канала "
        "с правом публикации и удаления сообщений."
    )
    await cb.answer()
...
async def m_h_token(message: Message, state: FSMContext) -> None:
    ...
    if ":" not in token or len(token) < 30:
        await message.answer("Не похоже на токен. Жду в формате <code>1234567:AA...</code>")
        return
    db = get_db()
    if await db.get_helper_by_token(token):
        await message.answer("Этот помощник уже добавлен.")
        return
    # верификация: создаём временного Bot, делаем get_me
    try:
    ...
    await message.answer(
        ...
        "Теперь добавь его админом канала с правами публикации/удаления, "
        "потом привяжи канал в разделе «Каналы».",
        reply_markup=helpers_menu(hs),
    )
...
async def cb_h_card(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    if not h:
        ...
        return
    db = get_db()
    chs = await db.list_channels(h_id)
    status = "🟢 работает" if h["is_alive"] else "💀 заморожен/удалён"
    err = ""
    if h["last_error"]:
        err = f"\n<i>Ошибка: {h['last_error'][:200]}</i>"
    await cb.message.edit_text(
