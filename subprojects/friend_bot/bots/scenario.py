...
class ScenarioEngine:
    ...
    async def _schedule_duplicate(self, bot: Bot, bot_record, user_id: int, step) -> None:
        ...
        if step["step_type"] == "message":
            cfg = json.loads(step["config"])
            # Дубли в шаге «Сообщение» имеют смысл ТОЛЬКО когда мы ждём
            # ответ юзера. При таймере шаг сам продвинется по времени,
            # при «без ожидания» — сразу advance. В обоих случаях
            # напоминать-дублировать нечего.
            if cfg.get("wait_mode") != "user_message":
                return
