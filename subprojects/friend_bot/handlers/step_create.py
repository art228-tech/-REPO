...
async def m_msg_content(message: Message, state: FSMContext) -> None:
    ...
    await state.update_data(draft=draft)
    # У скопированного поста были инлайн-кнопки — спросим что с ними
    if extracted.get("_orig_buttons"):
        from aiogram.types import InlineKeyboardButton, InlineKeyboardMarkup
        n = sum(len(r) for r in extracted["_orig_buttons"])
        await state.set_state(StepStates.msg_copy_buttons_choice)
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=f"✅ Оставить кнопки оригинала ({n})", callback_data="cpb:keep")],
            [InlineKeyboardButton(text="✏️ Задать свои кнопки", callback_data="cpb:own")],
            [InlineKeyboardButton(text="🚫 Без кнопок", callback_data="cpb:none")],
        ])
        await message.answer(
            f"✅ Пост принят. В нём <b>{n}</b> кнопок(и).\n\nЧто с ними делать?",
            reply_markup=kb,
        )
        return
...
@router.callback_query(StepStates.msg_copy_buttons_choice, F.data.startswith("cpb:"))
async def cb_copy_buttons_choice(cb: CallbackQuery, state: FSMContext) -> None:
    choice = cb.data.split(":")[1]
    data = await state.get_data()
    draft = data.get("draft", {})
    orig = draft.pop("_orig_buttons", None)
    if choice == "keep" and orig:
        ...
        await state.set_state(StepStates.msg_add_buttons)
        await cb.message.edit_text(
            "Добавим свои кнопки (со ссылками)?",
            reply_markup=add_button_now_kb(),
        )
    else:  # none
        draft["buttons"] = []
...
async def cb_msg_btn_color(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    data = await state.get_data()
    draft = data["draft"]
    btn = draft.pop("_current_button")
    btn["color"] = color
    draft.setdefault("buttons", []).append(btn)
    await state.update_data(draft=draft)
    await state.set_state(StepStates.msg_add_buttons)
    await cb.message.edit_text(
        f"✅ Кнопка добавлена.\nВсего: <b>{len(draft['buttons'])}</b>\n\n"
        f"Добавить ещё или закончить?",
        reply_markup=add_button_now_kb(),
    )
    await cb.answer()
